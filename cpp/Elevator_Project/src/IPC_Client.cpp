...
namespace IPC_Client_Unicast
{
	...
	int Client::accept_data_(const std::string msg_sent)
	{
		...
		ACE_Time_Value timout(1,0);/*wait for 3 seconds wait for acknowledge from server*/
	...
	int Client::accept_data_(const std::string msg_sent)
		{
			...
			ACE_Time_Value timeout(3,0);//500000);//us
			if(socket_.recv(this->data_buff_,this->dgram_byte_size_,this->remote_addr_,0,&timeout)!=-1)
			{
				data_string.assign(data_buff_,ACE_OS::strlen (this->data_buff_));
